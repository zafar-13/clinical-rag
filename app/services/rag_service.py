    ...
    def search(
        ...
        try:
            ...
            for hit in results[0]:
                chunks.append(
                    RetrievedChunk(
                        text=hit.entity.get("text_chunk"), score=hit.distance
                    )
                )
            return chunks
    ...
    def _ensure_collection_exists(self) -> Collection:
        ...
        if utility.has_collection(self.settings.milvus_collection_name):
            ...
            fields = [
                ...
                FieldSchema(
                    name="text_chunk", dtype=DataType.VARCHAR, max_length=65535
                ),
                FieldSchema(
                    name="vector",
            ...
            schema = CollectionSchema(
                fields, description="PDF Text Embeddings (Gemini)"
            )
            collection = Collection(
                name=self.settings.milvus_collection_name, schema=schema
            )
